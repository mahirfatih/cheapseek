...
- A collapsible **History** chart (last 7 days, peak vs. off-peak minutes).
...
- **History:** event-based — a sample is written only on a state change, in `UserDefaults`; entries are pruned to 7 days with a boundary anchor, and `HistoryAggregator` splits intervals into per-local-day peak/off-peak minutes (DST-sized 23h/25h days included).
...
- **Coverage:** `CheapSeek.app` line coverage **95.54%** (latest local full run) with a CI gate of **≥95%**.
